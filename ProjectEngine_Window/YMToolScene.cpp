...
namespace YM
{
	...
	void ToolScene::Initialize()
	{
		...
		CameraScript* camerasrt = camera->AddComponent<CameraScript>();
	...
	void ToolScene::LateUpdate()
	{
		...
		if (Input::GetKey(eKeyCode::LButton))
		{
			...
			pos = renderer::mainCamera->CalulateTilePosition(pos);
	...
	void ToolScene::Render(HDC hdc)
	{
		...
		{
			Vector2 pos = renderer::mainCamera->CalulatePosition(Vector2(TilemapRenderer::TileSize.x*i, 0.0f));
		...
		for (size_t i = 0; i < 50; i++)
		{
			Vector2 pos = renderer::mainCamera->CalulatePosition(Vector2(0.0f, TilemapRenderer::TileSize.y*i));
...
LRESULT CALLBACK WndTileProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
	{
		...
		YM::math::Vector2 mousePosition;
		mousePosition.x = mousePos.x;
		mousePosition.y = mousePos.y;
		int idxX = mousePosition.x / YM::TilemapRenderer::OriginTileSize.x;
		int idxY = mousePosition.y / YM::TilemapRenderer::OriginTileSize.y;
		YM::TilemapRenderer::SelectedIndex = Vector2(idxX, idxY);
	}
	break;
	case WM_PAINT:
	{
